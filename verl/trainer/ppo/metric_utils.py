...
def compute_risk_metrics(batch: DataProto, config: Any) -> dict[str, Any]:
    """
    Computes risk metrics for a batch of data.
    """
    sequence_score = batch.batch["token_level_scores"].sum(-1)
    if config is not None:
        quantile_down = config.get("quantile_down", 0.1)
        quantile_up = config.get("quantile_up", 0.9)
        metrics = {
            f"risk/quantile_{quantile_down}": torch.quantile(sequence_score, quantile_down).detach().item(),
            f"risk/quantile_{quantile_up}": torch.quantile(sequence_score, quantile_up).detach().item(),
        }
        return metrics
    else:
        metrics = {
            f"risk/quantile_{0.1}": torch.quantile(sequence_score, 0.1).detach().item(),
            f"risk/quantile_{0.9}": torch.quantile(sequence_score, 0.9).detach().item(),
        }
        return metrics
