...
from .code_utils import grade_answer_code
from .code_utils import extract_answer as code_extract_answer

from .math_utils.utils import grade_answer_sympy, grade_answer_mathd
from .math_utils.utils import extract_answer as math_extract_answer
...
class RewardCodeFn(RewardFn):
    ...
    def __call__(self, data_source: string, is_eval: bool, input: RewardInput):
        ...
        if data_source in ['livecodebench', 'livecodebench_v5']:
            # honor timeout from config
            timeout = 150
            is_correct = lcb_check_correctness(ground_truths, model_answer, timeout=timeout)
        else:
            is_correct, extro_info = grade_answer_code(model_answer, ground_truths, data_source, is_eval=is_eval)
            # # Run grade_answer_code in a subprocess to enforce timeout
            # manager = multiprocessing.Manager()
            # result_box = manager.list()

            # def _run_grade(model_ans, gts, ds, eval_flag, result_box_ref):
            #     try:
            #         res = grade_answer_code(model_ans, gts, ds, is_eval=eval_flag)
            #         result_box_ref.append(res)
            #     except Exception:
            #         # Any exception is treated as incorrect
            #         result_box_ref.append((False, None))

            # p = Process(target=_run_grade, args=(model_answer, ground_truths, data_source, is_eval, result_box))
            # p.start()
            # timeout = getattr(self.config, 'code_eval_timeout', 3)
