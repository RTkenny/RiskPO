        ...
        for parquet_file in self.data_files:
            ...
            try:
                ...
                try:
                    ...
                    raise e
            # 思路：
            # 这里的dataframe是通过pd.read_json读进来的pandas.DataFrame，而后续的代码需要的是datasets.Dataset类型。
            # 需要将pandas.DataFrame转换为datasets.Dataset，可以用datasets.Dataset.from_pandas方法。
            # 具体做法是在成功加载json为dataframe后，立刻将其转为datasets.Dataset。
            # 代码如下：
            if isinstance(dataframe, pd.DataFrame):
                dataframe = datasets.Dataset.from_pandas(dataframe, preserve_index=False)
